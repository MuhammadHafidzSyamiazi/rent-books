...
class AdminController extends Controller
{
    ...
    {
        $usersRegistered = User::where('roles_id', 2)->where('status', 'inavtive')->get();
    ...
    public function usersDetail($slug)
    {
        ...
        return view('admin.user.user-detail', ['users' => $user]);
    }
    ...
    {
        $book = Book::all();
        return view('admin.book.books', ['book' => $book]);
    }

    public function booksAdd()
    {
        $categories = Category::all();
        return view('admin.book.add-book', ['categories' => $categories]);
    }
